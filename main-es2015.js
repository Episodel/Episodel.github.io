...
class Task {
    constructor(id, name, status, check) {
        this.id = id;
        this.name = name;
        this.status = status;
        this.check = check;
...
let WorkAddUsersComponent = class WorkAddUsersComponent {
    constructor() {
        this.arrUsers = [];
    ...
    ngOnInit() { }
    addUser() {
        const newUser = new _shared_classes_userlist_model__WEBPACK_IMPORTED_MODULE_2__["User"](1, this.userLogin, this.userPassword, this.userEmail);
...
let WorkCenzorComponent = class WorkCenzorComponent {
    constructor() {
        this.title = "cenzor";
        this.arrBadWord = [];
        this.searchBad = [];
    ...
    censor() {
        if (this.textArea) {
            this.searchBad = this.textArea
                .replace(/[.,\/#?!$%\;;{}=\-_`~()]/g, "")
                .split(" ");
            this.arrBadWord.forEach(el => {
                this.searchBad.forEach((item, index) => {
                    if (el === item) {
                        item = item.replace(item, "*".repeat(item.length));
                        this.searchBad.splice(index, 1, item);
                    }
                });
            });
...
let WorkTasklistComponent = class WorkTasklistComponent {
    constructor() {
        this.arrTask = [
    ...
    ngOnInit() { }
    AddTask() {
        const newTask = new src_app_shared_classes_task_model__WEBPACK_IMPORTED_MODULE_2__["Task"](1, this.taskName, this.taskStatus, this.checkTask);
    ...
    deleteTask(task) {
        const index = this.arrTask.findIndex(ts => ts.id === task.id);
    ...
    editSave() {
        this.editTaskShow = !this.editTaskShow;
        this.arrTask.find(element => element.id === this.idTask ? (element.name = this.renameTask) : undefined);
    }
};
